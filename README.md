...
- WeChat mini-program-style mobile shell
- Photo upload preview with sticker/cutout board
- CapWords-inspired object card and collection flow
- Symptom chips tuned for thumb-first mobile input
- Plant archive with status tabs and two-column specimen cards
- Letter generation
- Rescue item list
- Step-by-step care plan
- API routes for diagnosis and letter generation

## Mini Program UX Direction

- First screen is the core action: take/upload a plant photo, then fill only the minimum care signals.
- Diagnosis output is a shareable mobile card: plant cutout, life score, status chip, funny first-person letter, rescue items, and step list.
- Archive is managed by plant, not by diagnosis record, so later versions can add recovery check-ins, death confirmations, and care reminders.
- Backend route handlers are intentionally separated from UI so they can later move to WeChat cloud functions or a standalone API.
